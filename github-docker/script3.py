...
with open('/auth/.github-oauth-token.json') as f:
...
def all_issues(repo):
  ...
  while True:
    ...
    for issue in issues_json:
      process_issue(issue) #this returns a pdf but we still need to put it somewhere!!!!!!!!!!!!!!!!!!!!!
    page += 1
...
def single_issue():
  # gets issue from command line (TBD)
  pass


def process_issue(issue_json):
  # make a temp directory write the markdown
  # convert the markdown
  # convert to pdf to same directory
  # return PDF path to caller

  tempdir = tempfile.mktemp()

  number = issue_json['number']
  pdf_filename = join(tempdir,'issue_{}.pdf'.format(number))
  ...
  md_content = ""
  md_content += "# #{0} – {1} \n\n".format(number, title)
  md_content += "### Reported by @{0} \n\n".format(issue_json['user']['login'])
  
  if issue_json['milestone']:
      md_content += '**Milestone**: {0} \n\n'.format(issue_json['milestone']['title'])
  ...
  md_content += body
  md_content += "\n\n"
  if issue_json['comments'] > 0:
      comments_request = requests.get(issue_json['comments_url'],
                                      headers=standard_headers)
  ...
  # render to .tex file
  md_file_path = tempdir + "issue_{}.md".format(number)
  file = open(md_file_path, "w+")
  file.write(md_content)
  file.close()

  tex_file_path = tempdir + "issue_{}.tex".format(number)
  ...
  subprocess.check_call(['xelatex', '-output-directory={}'.format(tempdir), tex_file_path])

  pdf_file_path = tempdir + "issue_{}.pdf".format(number)
  issue_pdf = open(pdf_file_path, 'r')
  return (issue_pdf)
